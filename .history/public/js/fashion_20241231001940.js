 ...
 function addToFavorite(productId) {
     ...
     // Kiểm tra giá trị của userId và productId
     ...
     fetch('index.php?controller=favorite&action=addToFavorite', {
         ...
         .then(({ success, message }) => {
             console.log('Phản hồi từ server:', success, message); // In ra kết quả trả về từ server
             if (success) {
                 alert('Thêm vào mục yêu thích thành công!');
             } else {
                 alert(message); // Thông báo lỗi từ server
             }
         })
         .catch((err) => {
             console.error('Lỗi:', err); // Kiểm tra lỗi trong console
         });
 }