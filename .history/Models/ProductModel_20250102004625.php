...
class ProductModel extends BaseModel
{
    ...
    {
        $stmt = $this->connect->prepare("INSERT INTO " . self::FAVORITE_TABLE_NAME . " (user_id, product_id) VALUES (?, ?)");
    ...
    {
        $stmt = $this->connect->prepare("DELETE FROM " . self::FAVORITE_TABLE_NAME . " WHERE user_id = ? AND product_id = ?");
        $stmt->bind_param("ii", $userId, $productId);
        return $stmt->execute();
    }
    ...
    {
        $stmt = $this->connect->prepare("SELECT p.* FROM " . self::FAVORITE_TABLE_NAME . " f INNER JOIN " . self::TABLE_NAME . " p ON f.product_id = p.product_id WHERE f.user_id = ?");
    ...
    {
        $stmt = $this->connect->prepare("SELECT * FROM " . self::FAVORITE_TABLE_NAME . " WHERE user_id = ? AND product_id = ?");
        $stmt->bind_param('ii', $userId, $productId);
        $stmt->execute();
