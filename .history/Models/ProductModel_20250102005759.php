...
class ProductModel extends BaseModel
{
    ...
    // Thêm sản phẩm vào danh sách yêu thích
    public function addToFavorite($product_id, $user_id)
    {
        // Kiểm tra nếu sản phẩm đã có trong danh sách yêu thích của người dùng
        $stmt = $this->connect->prepare("SELECT * FROM favorite WHERE product_id = ? AND user_id = ?");
        $stmt->bind_param("ii", $product_id, $user_id);
        $stmt->execute();
        $result = $stmt->get_result();

        if ($result->num_rows > 0) {
            // Nếu sản phẩm đã có, không thêm nữa
            return false;
        } else {
            // Nếu chưa có, thêm sản phẩm vào bảng favorite
            $stmt = $this->connect->prepare("INSERT INTO favorite (product_id, user_id, tfavorite_view_at) VALUES (?, ?, NOW())");
            $stmt->bind_param("ii", $product_id, $user_id);
            return $stmt->execute();
        }
    }
    ...
    public function removeFromFavorite($product_id, $user_id)
    {
        $stmt = $this->connect->prepare("DELETE FROM favorite WHERE product_id = ? AND user_id = ?");
        $stmt->bind_param("ii", $product_id, $user_id);
        return $stmt->execute();
    }
    ...
    {
        $stmt = $this->connect->prepare("SELECT p.product_id, p.product_name, p.price, p.product_image FROM products p
