...
class ProductModel extends BaseModel
{
    ...
    public function getFavoritesByUser($userId)
    {
        $query = "
            SELECT products.* 
            FROM favorite
            INNER JOIN products ON favorite.product_id = products.product_id
    ...
    public function addToFavorite($userId, $productId)
    {
        ...
        $query = "SELECT * FROM favorite WHERE user_id = ? AND product_id = ?";
        $stmt = $this->connect->prepare($query);
        ...
        if ($result->num_rows > 0) {
            return false; // Nếu đã có trong danh sách yêu thích thì không thêm
        }

        $insertQuery = "INSERT INTO favorite (user_id, product_id) VALUES (?, ?)";
        $insertStmt = $this->connect->prepare($insertQuery);
    ...
    {
        $query = "DELETE FROM favorite WHERE user_id = ? AND product_id = ?";
        $stmt = $this->connect->prepare($query);
        $stmt->bind_param("ii", $userId, $productId);
        return $stmt->execute();
    }
