...
class ProductModel extends BaseModel
{
    ...
    public function addFavorite($userId, $productId)
    {
        ...
        return $this->connect->execute($sql, [$userId, $productId]);
    }

    // Lấy danh sách sản phẩm yêu thích của người dùng
    public function getFavorites($userId)
    {
        $sql = "SELECT f.favorite_id, p.product_name, p.product_image, p.price, p.type 
                FROM favorite f 
                JOIN products p ON f.product_id = p.product_id 
                WHERE f.user_id = ?";
        return $this->connect->fetchAll($sql, [$userId]);
    }
    ...
    public function removeFavorite($favoriteId, $userId)
    {
        ...
        return $this->connect->execute($sql, [$favoriteId, $userId]);
    }
}
