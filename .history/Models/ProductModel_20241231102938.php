...
class ProductModel extends BaseModel
{
    ...
    public function checkFavorite($user_id, $product_id) {
        $stmt = $this->connect->prepare("SELECT * FROM favorite WHERE user_id = ? AND product_id = ?");
    ...
    public function addFavorite($user_id, $product_id) {
        $columns = ['user_id', 'product_id', 'tfavorite_view_at'];
        $values = [$user_id, $product_id, date('Y-m-d H:i:s')];
        return $this->store($columns, $values);
    }

    public function removeFavorite($user_id, $favorite_id) {
        $stmt = $this->connect->prepare("DELETE FROM favorite WHERE user_id = ? AND favorite_id = ?");
