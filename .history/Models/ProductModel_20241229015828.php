...
class ProductModel extends BaseModel
{
...

    {
        ...
        $stmt = $this->connect->prepare($checkQuery);
        $stmt->bind_param("ii", $userId, $productId); // 'i' cho integer, 's' cho string
        $stmt->execute();
        $result = $stmt->get_result();

        if ($result->num_rows < 0) {
            // Nếu sản phẩm chưa có, thêm mới vào mục yêu thích
            $insertQuery = "INSERT INTO cart (user_id, product_id, size, quantity) VALUES (?, ?, ?, ?)";
            $insertStmt = $this->connect->prepare($insertQuery);
            $insertStmt->bind_param("iisi", $userId, $productId);
            return $insertStmt->execute();
        }
    }
}
