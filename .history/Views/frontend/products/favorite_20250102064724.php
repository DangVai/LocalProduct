...
<?php endif; ?>
    ...
    <script>
        ...
        function removeFromFavorites(productId) {
            ...
            if (userId === "null") {
                alert("Bạn cần đăng nhập để xóa sản phẩm khỏi danh sách yêu thích!");
                return;
            }

            // Gửi yêu cầu tới server để xóa sản phẩm khỏi danh sách yêu thích
            fetch(`/index.php?controller=favorite&action=remove`, {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.status === 'success') {
                        alert(data.message);
                        document.querySelector(`[data-product-id="${productId}"]`).parentNode.remove();
                    } else {
                        alert(data.message);
                    }
                })
