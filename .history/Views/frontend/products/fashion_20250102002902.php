...
<body>
            ...
            <?php if (!empty($products)): ?>
                ...
                <?php foreach ($products as $product): ?>
                    <div class="product-item" data-price="<?= $product['price'] ?>" data-type="<?= htmlspecialchars($product['type']) ?>" data-name="<?= htmlspecialchars($product['product_name']) ?>">
                        <button class="favourite-btn" data-id="<?= $product['product_id'] ?>" onclick="addToFavorite(<?= $product['product_id'] ?>)">
    ...
    <script>
        ...
        function toggleFavorite(productId) {
            ...
            // Gửi yêu cầu tới server để thêm/xóa sản phẩm vào yêu thích
            fetch(`/index.php?controller=favorite&action=toggle`, {
                ...
                .then(data => {
                    if (data.status === 'success') {
                        alert(data.message);
                        document.querySelector(`[data-product-id="${productId}"]`).classList.toggle('active');
                    } else {
                        alert(data.message);
                    }
                })
